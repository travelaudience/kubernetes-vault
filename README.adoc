...
* One first creates the Vault `StatefulSet` itself, which creates two Vault
  instances that are _unitialized_ and _sealed_. This means they will not
...
[source,bash]
----
$ kubectl create -f vault/vault-configmap.yaml // <1>
configmap "vault" created
----
<1> Before running this command, one should edit `vault-configmap.yaml` and
replace the value of `redirect_addr` with the address that will be used to
...
As mentioned above, this will create two Vault instances that are _unitialized_
and _sealed_. This means that they will not accept requests except for the ones
required for the initial configuration process.
...
the first terminal window — where `kubectl port-forward` is running — and stop
the running process (using `Ctrl-C`). Then, one should run
...
Now, one should get back to the second terminal window — where `vault init` and
`vault unseal` were run — and run the following commands:
...
$ kubectl get --namespace vault pod
NAME      READY     STATUS    RESTARTS   AGE
vault-0   2/2       Running   0          15m
vault-1   2/2       Running   0          15m
----
